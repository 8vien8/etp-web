...
import { Card, CardBody, CardTitle, Button, Modal, ModalHeader, ModalBody, ModalFooter, CardSubtitle, Input, Label } from 'reactstrap';
...
function Directors() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const coordinatorData = data.filter(user => user.role === 'coordinator');
    ...
    const confirmSave = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:3001/users/${editedCoordinator.id}`, {
                method: 'PUT',
                headers: {
    ...
    return (
        <div>
                ...
                {coordinators.map(coordinator => (
                    <Card key={coordinator.id}>
                        <CardBody className='articles-container'>
                            ...
                            <Button onClick={() => showCoordinatorDetails(coordinator)}>Detail</Button>
                        </CardBody>
                    </Card>
                ))}
            </div>
                    ...
                    {selectedCoordinator && (
                        <div>
                            <div style={{ display: 'flex', justifyContent: 'center' }} >
                                <img style={{ width: '160px', height: '160px', borderRadius: '50%' }} src={editedCoordinator.picture} alt={editedCoordinator.username} />
                            </div>
                            <h4 style={{ marginBottom: "unset", display: "flex", justifyContent: "center" }}>{selectedCoordinator.code} </h4>
                            <Label for="username"><strong>Username</strong></Label>
                            <Input type="text" name="username" id="username" value={editedCoordinator.username || ''} onChange={handleInputChange} />
                            <Label for="email"><strong>Email</strong></Label>
                            <Input type="email" name="email" id="email" value={editedCoordinator.email || ''} onChange={handleInputChange} />
            ...
            <Modal isOpen={confirmModalOpen} toggle={toggleConfirmModal}>
                ...
                <ModalFooter>
                    <Button color="primary" onClick={confirmSave}>Save</Button>
